...
class PackageDistributionLink(SQLModel, table=True):
    ...
    distribution_id: int | None = Field(default=None, foreign_key="distribution.id", primary_key=True)
...
class PackageArchitectureLink(SQLModel, table=True):
    ...
    package_id: int | None = Field(default=None, foreign_key="package.id", primary_key=True)
    architecture_id: int | None = Field(default=None, foreign_key="architecture.id", primary_key=True)
